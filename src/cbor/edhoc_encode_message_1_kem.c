...
static bool encode_message_1_kem(
	zcbor_state_t *state, const struct message_1_kem *input);
	...
	zcbor_state_t *state, const struct message_1_kem *input)
{
	...
	: false)))
	&& ((zcbor_bstr_encode(state, (&(*input).pk_eph))))
		...
		size_t *payload_len_out)
{
	...
	return zcbor_entry_function(payload, payload_len, (void *)input, payload_len_out, states,
		(zcbor_decoder_t *)encode_message_1_kem, sizeof(states) / sizeof(zcbor_state_t), 5);
}
