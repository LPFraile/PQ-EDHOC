...
static bool decode_message_1_kem(zcbor_state_t *state, struct message_1_kem *result);


static bool decode_message_1_kem(
		zcbor_state_t *state, struct message_1_kem *result)
{
	zcbor_log("%s\r\n", __func__);
	bool int_res;
	...
	&& ((zcbor_bstr_decode(state, (&(*result).pk_eph))))
		...
		size_t *payload_len_out)
{
	...
	return zcbor_entry_function(payload, payload_len, (void *)result, payload_len_out, states,
		(zcbor_decoder_t *)decode_message_1_kem, sizeof(states) / sizeof(zcbor_state_t), 5);
}
