 ...
 *			
 * @param alg			The KEM to be used. KYBER_LEVEL1, KYBER_LEVEL3 or KYBER_LEVEL5 
 * @param[out] sk 		The newly generated private key.
...
/**
	 ...
	 *
     * @param[in] pk The public key represented as a byte string.
	 * @param[in] msg The message represented as a byte string.
	 * @param[in] signature The signature on the message represented as a byte string.
	 	 * @return OQS_SUCCESS or OQS_ERROR
	 */
enum err sign_verify(enum sign_alg alg, const struct byte_array *pk,
	       const struct byte_array *msg,
