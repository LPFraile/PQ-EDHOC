                 ...
                 bool last_block, void *user_data)
{
    ...
    if (code >= 0) {
			...
			if (code == 68){
				PRINT_MSG("ACk 2.04 changed\n");
				memcpy(my_buffer + ptr_len ,payload,len);
				ptr_len = ptr_len + len;
				if(len == 0){
					PRINT_MSG("ACk 2.04 changed without block 2\n");
				}
				else{
					PRINT_MSG("ACk 2.04 changed with block 2\n");
...
static int txrx_edhoc(int sockfd)
{
	...
	if (k_sem_take(&my_sem_tx, K_FOREVER) != 0) {
		...
		PRINT_MSG("Send the messages MSG1\n");
	...
	if (k_sem_take(&my_sem_tx, K_FOREVER) != 0) {
		...
		//k_sleep(K_MSEC(500));
		PRINT_MSG("Send the messages MSG3\n");
